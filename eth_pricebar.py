def test_display():
    ...
    try:
        ...
        print("\nGenerating ETH Price Update:")
        print(f"Current ETH Price: ${eth_price:,.2f}")
        print(f"Price Change: {change_text}")
        print(f"ETH/BTC Ratio: {eth_btc_ratio:.3f}")
        
        # Add final verification
        if status:
            print("Successfully generated ETH status")
            print(status)
            return status
        else:
            print("Failed to generate ETH status")
            return None
            
    except Exception as e:
        print(f"Error in ETH price display: {e}")
        return None
